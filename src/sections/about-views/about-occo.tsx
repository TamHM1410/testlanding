...
export default function AboutOcco() {
  return (
    <> 
    <div className="min-w-screen max-w-screen py-5 flex justify-center 2xl:text-2xl">

    <div className="lg:grid lg:grid-cols-2 lg:gap-5 py-5 mx-[10%] lg:h-[527px] min-h-fit max-lg:flex max-lg:flex-col-reverse  max-w-[1440px]">
        <div
          className=" flex items-center  max-lg:pt-10 
        "
        >
          <Image src={double} alt="" height={400} width={600} />
        </div>
        <div className=" h-[400px] pt-5 text-white text-left gap-y-[20]  max-lg:text-center max-sm:h-full">
          ...
          {Content.aboutContent.aboutOcco_des &&
            Content.aboutContent.aboutOcco_des.map((item) => {
              return (
                <>
                  <div key={item} className="pt-5 text-customGrey ">{item}</div>
                </>
              );
            })}
        </div>
